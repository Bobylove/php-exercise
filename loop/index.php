
<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>loop</title>
</head>
<body>
	<h1 class="exo1">
		exo 1 loop
		<?php  include "../loop/exercice1.php"; ?> 
	</h1>
	<br>

	<h1 class="exo2">exo 2 loop 
		<?php include "../loop/exercice2.php"; ?>
	</h1>
	<br>


	<h1 class="exo3">exo 3 loop 
		<?php include "../loop/exercice3.php"; ?>
	</h1>
	<br>


	<h1 class="exo4">exo 4 loop 
		<?php include "../loop/exercice4.php"; ?>
	</h1>
	<br>

	<h1 class="exo5">exo 5 loop 
		<?php include "../loop/exercice5.php"; ?>
	</h1>
	<br>

	<h1 class="exo6">exo 6loop 
		<?php include "../loop/exercice6.php"; ?>
	</h1>
	<br>

	<h1 class="exo7">exo 7loop 
		<?php include "../loop/exercice7.php"; ?>
	</h1>
	<br>

<h1 class="exo8">exo 8loop 
		<?php include "../loop/exercice8.php"; ?>
	</h1>
	<br>



</body>
</html>